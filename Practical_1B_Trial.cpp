...
int main(void)
{
  ...
  checksum = calculate_mandelbrot_double(160, 160, MAX_ITER);
 
  //TODO: Record the end time
  end_time = HAL_GetTick();
 
  //TODO: Calculate the execution time
  execution_time = end_time - start_time; //Execution time is just the difference between the start and end time//
...
uint64_t calculate_mandelbrot_fixed_point_arithmetic(int width, int height, int max_iterations){
    ...
    for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                mandelbrot_sum += iteration; //THIS IS THE CHECKSUM 
            }
        }
